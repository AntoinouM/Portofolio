...
export default class ResourceManager extends EventTarget {
  ...
  manageResources(directory, listOfElements) {
    if (Array.isArray(directory)) {
      ...
      this.manageResourcesArrayOfDir(directory).then(() => {
        this.emitEnd();
      });
    } else {
      // manage single
      this.manageResourcesSingleDir(directory, listOfElements).then(() => {
        this.emitEnd();
      });
    }
  }

  manageResourcesArrayOfDir(directories) {
    return new Promise((resolve, reject) => {
      ...
      Promise.all(promisesOfArrayDir).then(() => {
        resolve(
          `All resources have been loaded (${this.images.size} images, ${this.videos.size} videos)`
        );
      });
    });
  }
  ...
  manageResourcesSingleDir(directory, listOfElements) {
    return new Promise((resolve, reject) => {
      this.declareResources(directory, listOfElements).then(() => {
        this.handleResourcesLoading().then((value) => {
          resolve(value);
        });
      });
    });
    ...
    return new Promise(async (resolve, reject) => {
      ...
      checkFiles: for (let i in files) {
        ...
        let resource = new Resource(
          this.getFormat(resourceName),
          this.toKey(resourceName),
          this.toSrc(resourceName, directory),
          directory
        );
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.images.size !== 0) {
        this.images.forEach((value, key) => {
          promisesList.push(this.handleSingleResourceLoading(value));
        });
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      switch (resource.type) {
        case 'image': {
          ...
          image.onerror = () => {
            reject(
              `Impossible to load image ${resource.key} from (${resource.src}).`
            );
          };
        ...
        case 'video': {
          ...
          video.onerror = () => {
            reject(
              `Impossible to load video ${resource.key} from (${resource.src}).`
            );
          };
        ...
        default:
          this.cl(`Resource ${resource.src} type is not recognized.`);
      }
    });
  }
   ...
   */
  getFilteredArray(p) {
    let generalArray = Array.from(this.images.values()).concat(
    ...
    this.dispatchEvent(
      new CustomEvent('start', {
        detail: {
          ...
          message: `Resource loading starts (${
            this.images.size + this.videos.size
          } resources.)`,
        },
      })
    );
